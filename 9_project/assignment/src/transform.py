...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    merged_items_orders = pd.merge(items, orders, on='order_id', how='inner')
    data = pd.merge(merged_items_orders, products, on='product_id', how='inner')
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date = (filtered_dates.groupby(filtered_dates["order_purchase_timestamp"].dt.date)
                                       .size()
    ...
    # Asegúrate de que las columnas en holidays y order_purchase_ammount_per_date tengan el mismo tipo de dato para la comparación
    order_purchase_ammount_per_date["date"] = pd.to_datetime(order_purchase_ammount_per_date["order_purchase_timestamp"])
    holidays["date"] = pd.to_datetime(holidays["date"])

    # Agregar la columna 'holiday' verificando si la fecha está en holidays
