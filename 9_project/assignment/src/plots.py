...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    plt.title("Relación entre Peso del Producto y Valor del Flete", fontsize=14)
    plt.xlabel("Peso del Producto (g)", fontsize=12)
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.plot(df['date'], df['order_count'], color='blue', label='Orders')

    #Marcar los días festivos con líneas verticales rojas
    for holiday_date in df[df['holiday'] == True]['date']:
        plt.axvline(x=holiday_date, color='red', alpha=0.3, linestyle='--')
